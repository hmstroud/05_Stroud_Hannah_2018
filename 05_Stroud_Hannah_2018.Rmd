...
sqrt((p*(1-p))/30)
```
The standard error tells us that something......  
...
#some kind of fucking t test is the answer t.test(species$speciesUpstream~species$speciesDownstream, data= species, alternative = "two.sided")
```
...
#compare to power.t.test (a function we never reviewed in class)

power.t.test(25, 3, 2)
```
I got a much higher power, but I honestly don't understand how to get this function to work since there are two standard deviations and the argument for this test only wants one. 
